...
GPIO.setup(CROUCH_BUTTON_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
GPIO.setup(BOMB_SWITCH_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)

bomb_switch_prev_state = GPIO.input(BOMB_SWITCH_PIN)
...
JOYSTICK_DEADZONE_X = 2780
JOYSTICK_DEADZONE_Y = 2850
THRESHOLD = 200 

bomb_switch_prev_state = GPIO.input(BOMB_SWITCH_PIN)

def input():
    global run, bomb_switch_prev_state
    ...
    bomb_switch_curr_state = GPIO.input(BOMB_SWITCH_PIN)

    if bomb_switch_curr_state != bomb_switch_prev_state:
        app.bomb = True 
        bomb_switch_prev_state = bomb_switch_curr_state
    else:
        app.bomb = False
        app.bomb_thrown = False

    if GPIO.input(CROUCH_BUTTON_PIN) == GPIO.LOW:
        app.crouched = True
