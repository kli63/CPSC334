...
def vectorise(
    ...
    draw_hatch=False,
    repeat_hatch=1,
    svg_folder=None,  # Default to None to enforce dynamic usage
    json_folder=None
):
    # Ensure paths are explicitly set
    if not svg_folder or not json_folder:
        raise ValueError("Both svg_folder and json_folder must be specified.")

    # Ensure the directories exist
    os.makedirs(svg_folder, exist_ok=True)
    os.makedirs(json_folder, exist_ok=True)

    # Attempt to load the image from potential paths
    possible = [
        image_filename,
        os.path.join(svg_folder, image_filename + ".jpg"),
        os.path.join(svg_folder, image_filename + ".png"),
        os.path.join(svg_folder, image_filename + ".tif"),
    ]
    image = None
    for p in possible:
        try:
    ...
    if not image:
        raise FileNotFoundError(f"Image {image_filename} not found in {svg_folder} or related paths.")

    w, h = image.size

    # Convert to greyscale and maximise contrast
    ...
    print(f"{len(lines)} lines, {segments} segments.")

    # Save SVG and JSON files
    svg_path = os.path.join(svg_folder, f"{image_filename}.svg")
    json_path = os.path.join(json_folder, f"{image_filename}.json")

    # with open(svg_path, "w") as svg_file:
    #     svg_file.write(makesvg(lines))
    # print(f"SVG saved to: {svg_path}")

    # lines_to_file(lines, json_path)
